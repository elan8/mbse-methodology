...
| **Engineering increment** | A vertical slice connecting need → scenario → requirement → architecture → analysis → verification → decision. | An agile sprint (timebox). An increment may span one or more sprints. |
...
| **Tailoring profile** | `small` / `medium` / `regulated` recorded in `ProjectTailoring`. | Skipping all modeling for “agile.” |
